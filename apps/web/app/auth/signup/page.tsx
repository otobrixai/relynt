...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (!orgId) {
      setError('Failed to create organization: No ID returned')
      setLoading(false)
      return
    }
        ...
        >
          ...
          <div className="flex flex-col items-center mb-10 text-center">
            ...
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] bg-white/5 px-4 py-1.5 rounded-full border border-white/5">
              Initialize Governance Boundary
            </p>
          </div>
            ...
            <form onSubmit={handleSignup} className="space-y-5 relative z-10">
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Entity Identity</label>
                  <div className="relative group">
                    <Building className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-cyan-400 transition-colors" />
                    <input
                      type="text"
              ...
              >
                {loading ? (
                  <div className="flex items-center justify-center gap-3">
                    ...
                    <span className="text-xs font-black uppercase tracking-widest">Provisioning...</span>
                  </div>
                ) : (
                  <div className="flex items-center justify-center gap-2">
                    <span className="text-xs font-black uppercase tracking-widest">Establish Boundary</span>
              ...
              <div className="text-center mt-6">
                <a href="/auth/login" className="text-[10px] font-black text-cyan-400 hover:text-cyan-300 uppercase tracking-widest transition-colors inline-flex items-center gap-2 group">
          ...
          <p className="mt-8 text-center text-[9px] font-black text-slate-600 uppercase tracking-[0.4em]">
            &copy; 2024 Relynt Intelligence Operations.
          </p>
        </motion.div>
      </div>
    </div>
